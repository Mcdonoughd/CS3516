//node2.c by Daniel McDonough 10/6/18
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "project3.h"
...
int prev_DV_2[MAX_NODES]; //array that keeps the a cached DV in respect to Node0. INIT =  {3,1,0,2}
...
void send_pkt_2(){
  ...
  int i;
   for( i=0;i<MAX_NODES;i++) {
       ...
       rpkt_2[i].destid = i; //
       memcpy(rpkt_2[i].mincost, prev_DV_2, sizeof(prev_DV_2)); //make a copy of the pacets
   }

   // SEND TO ALL NEIGHBOURS
  
   for( i=0 ;i<MAX_NODES;i++) {
       if(i!=Node2_ID) {   //not sending to itself
           toLayer2(rpkt_2[i]);
...
void rtinit2() {
    ...
    for( i=0;i<MAX_NODES;i++){
       for( j=0;j<MAX_NODES;j++){
         if(i==j){
           dt2.costs[i][j] =  ConnectCosts[Node2_ID][j]; //the diagonals are direct neightbors

           //init the DV for iteself
           if(i==Node2_ID){
             prev_DV_2[Node2_ID] = 0;//always 0 in reference to itself!
           }else{
             prev_DV_2[i] = dt2.costs[i][j];//its equivalent to the diagonals initally
           }

         }
        else{
          dt2.costs[i][j] = INFINITY; //everything else is unknown
          }
      }
    }
